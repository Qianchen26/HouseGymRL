...
class BaseEnv(gym.Env):
    ...
    def _calculate_reward(self, completed: List[int]) -> Dict[str, float]:
        ...
        progress_bonus = 1.0 + (current_completed / max(num_revealed, 1)) * 0.5
        progress_reward = progress * progress_bonus
        
        if completion < 0.5:
            completion_reward = completion * 2.0
        else:
            completion_reward = 1.0 + (completion - 0.5) * 1.0
        ...
        if queue_size > 0:
            ...
            queue_penalty_per_house = self._queue_penalty_sigmoid(
                int(avg_waiting_time),
                midpoint=40,
                steepness=0.03,
                max_penalty=0.03
            )
            queue_penalty = queue_penalty_per_house * (queue_size / max(num_revealed, 1)) * 0.5
            queue_penalty = max(queue_penalty, -1.0)
        else:
            queue_penalty = 0.0

        time_remaining = max(0, self.max_steps - self.day)
        time_pressure_bonus = (time_remaining / self.max_steps) * 0.1
